...
function ResourceCard(item: AccessibilityResource) {
  ...
  // ✅ Buttons wie vorher im Light; im Dark passend wie Apple
  const controlBtnCls = isDarkTile
    ? "bg-white text-slate-900 ring-1 ring-black/10 hover:bg-white/95"
    : "bg-slate-900 text-white ring-1 ring-white/10 hover:bg-slate-800 dark:bg-black dark:text-white dark:ring-1 dark:ring-black/10 dark:hover:bg-black/95";

  const iconCls = isDarkTile ? "text-white" : "text-slate-900 dark:text-white";
  const descCls = isDarkTile
    ? "text-white/85"
    : "text-slate-700 dark:text-white/85";

  // Link-Card soll sich wie ein Link verhalten (wie gehabt)
  const CardInner = (
    <div
      className={[
      ...
      <div className="relative z-10 p-8">
        <div className="flex flex-col gap-5">
          ...
          <div className="space-y-3">
            ...
            <p className={["text-[17px] leading-[1.35]", descCls, "max-w-[34ch]"].join(" ")}>
              {item.description}
            </p>
          </div>
        </div>
  ...
  return (
    <section className="relative dark:bg-black">
      <div className="mx-auto max-w-6xl px-6 pt-[56px] md:pt-[88px] pb-[72px] md:pb-[120px]">
        ...
        <div className="[--card-w:277px] [--card-h:322px] sm:[--card-w:350px] sm:[--card-h:333px] lg:[--card-w:350px] lg:[--card-h:333px]">
